...
contract PickupTokenTest is Test {
    ...
    function setUp() public {
        token = new PickupToken("PickupToken", "PUK");
    }
    ...
    function testMintUpdatesTotalMinted() public {
        uint256 before = token.totalMinted();
        uint256 amount = 500 * 10 ** token.decimals();
        token.mint(customer, amount);
        assertEq(token.totalMinted(), before + amount);
    }
    ...
    function testCannotBurnFromOtherWithoutAllowance() public {
        vm.prank(agent);
        vm.expectRevert();
